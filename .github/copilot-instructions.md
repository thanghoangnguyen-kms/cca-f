...
├── AGENTS.md · CLAUDE.md · README.md ← agent instructions (vault root)
├── skills-lock.json                 ← pinned external skill (baoyu-youtube-transcript)
├── .claude/                         ← vault-local skills (mermaid-diagrams, claude-md-improver)
└── .github/copilot-instructions.md
```

Domain notes are named `D<N> - <Topic>.md`; course notes `C<N> - <Course>.md`; episode notes `EP<NN> - <Title>.md`.
...
Generated by the **`/cca-episode <transcript-path>`** command, which owns the full procedure:
...
3. **Keep emoji out of `##`/`###` headings** so the Outline's anchor links resolve.
4. **Verify every exam-critical claim against official docs** — transcripts contain real errors. **Never silently correct the lecture**; show both sides in a `[!WARNING]` callout and name the exam-safe answer.
5. Cross-check the vault's own `D1`–`D5` notes; if they disagree with a transcript, the vault usually wins — say so explicitly.
...
**Two tiers of deck, and they do not mix:**

| Deck | Scope | Rule |
|------|-------|------|
| `04 - Exam Cram/Flashcards.md` | Vault-wide, grouped by domain | **Never append episode cards here.** Edit a card in place only when it has gone *stale* |
| `06 - Youtube Course/EP<NN> - .../EP<NN> - Flashcards.md` | One episode | Dedupe against the vault-wide deck; name skipped topics in the deck's header callout |
...
> - **The bundled `claude-api` skill does not cover the Claude Agent SDK.** `AgentDefinition`, subagents, and the `Task`/`Agent` tool live in the Agent SDK docs and must be fetched separately.

### Known corrections to third-party material

Recheck these whenever a topic touches them — the YouTube course and older practice banks get them wrong:
...
- **Do not** create study content outside the vault root (`/Users/thanghoangnguyen/Documents/Obsidian Vault/CCA-F/`) — agent tooling (slash commands, skills, settings) belongs in `~/.claude/` or the vault's `.claude/` and is exempt
