...
Domain notes are named `D<N> - <Topic>.md`; course notes `C<N> - <Course>.md`. Because Obsidian resolves `[[wikilinks]]` by **filename**, notes can move between folders without breaking links — but the `Handbook/` files use relative `[text](file.md)` links, so keep that folder intact as a unit.
