...
> **Drilling as multiple choice** → `New Mock Exam` (the only set quoting all four options). **Settling what the right answer is** → `Timed Mock 2026-08-24` (every answer is the site's own `correct_key`). **Your own scored performance** → `Timed Mock 2026-08-24`; it is the only sitting whose result is recorded.
